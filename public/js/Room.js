...
async function whoAreYou() {
    ...
    async function fetchNameByID(id) {
        try {
            ...
            return data.userId || ''; // Default to empty string if `userId` is missing
        } catch (error) {
    ...
    async function JoinMeetingApi(ID, MI) {
        try {
    ...
    async function StartMeetingApi(ID, MI) {
        try {
    ...
    if (ID) {
        default_name = await fetchNameByID(ID);
    }
    ...
    Swal.fire({
        ...
        if (H == 1) {
            await StartMeetingApi(ID, MI);
            getPeerInfo();
            joinRoom(peer_name, MI);
        } else {
            await JoinMeetingApi(ID, MI);
            getPeerInfo();
            joinRoom(peer_name, MI);
        }
    });
