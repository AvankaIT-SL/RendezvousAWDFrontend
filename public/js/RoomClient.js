...
class RoomClient {
    ...
    async checkIfInMeeting() {
        ...
        const ID = urlParams.get('ID'); // Invited user ID
        const MI = urlParams.get('room'); // Initiating user ID
        const H = urlParams.get('H'); // Initiating user ID
        ...
        if (H == 1) {
            ...
            this.meetingCheckInterval = setInterval(() => {
                fetch('https://rendezvousbackend.onrender.com/updateHostLeaveTime', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({
                        ID, // Current user (initiated to)
                        MI,    // Meeting ID
                    }),
                })
            ...
            // Save the interval ID to clear it later if needed
            this.meetingCheckInterval = setInterval(() => {
                fetch('https://rendezvousbackend.onrender.com/updateParticpantLeaveTime', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    async exitRoom() {
        ...
        const ID = urlParams.get('ID'); // Invited user ID
        const H = urlParams.get('H'); // Initiating user ID
        const MI = urlParams.get('room'); // Extract MeetingID from the path
        ...
        try {
            ...
            if(H == 1) {
                ...
                try {
                    const response = await fetch(`https://rendezvousbackend.onrender.com/HostMeetingStatus`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({ ID, MI, S: 3 }),
                    });
                    if (!response.ok) {
                ...
                try {
                    const response = await fetch(`https://rendezvousbackend.onrender.com/ParticipantMeetingStatus`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({ ID, MI, S: 5 }),
                    });
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }
    ...
    editorUnpin() {
        ...
        this.pollCenter();
        this.isEditorPinned = false;
        setColor(editorTogglePin, 'black');
        this.resizeVideoMenuBar();
        resizeVideoMedia();
